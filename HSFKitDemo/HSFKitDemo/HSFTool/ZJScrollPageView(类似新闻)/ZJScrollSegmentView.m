...
// 滚动条+箭头
@property (strong, nonatomic) CAShapeLayer *scrollLineArrowLayer;
// 遮盖
...
- (void)addScrollLineOrCoverOrExtraBtn {
    /** 是否显示底部baseline 默认为NO */
    if (self.segmentStyle.showBaseline) {
        if (self.segmentStyle.showLineArrow) {
            self.baseline = [[UIView alloc]initWithFrame:CGRectZero];
            self.baseline.backgroundColor = [UIColor groupTableViewBackgroundColor];
            [self.scrollView addSubview:self.baseline];
        }
    }
...
- (void)setupScrollLineAndCover {
    ...
    if (self.scrollLine) {
        ...
        if (self.segmentStyle.isScrollTitle) {
            ...
            if (self.segmentStyle.showBaseline) {
                if (self.segmentStyle.showLineArrow) {
                    self.baseline.frame = CGRectMake(0, self.scrollLine.y, self.zj_width, self.scrollLine.zj_height/3);
                }else{
                    self.baseline.frame = CGRectMake(0, self.scrollLine.y, self.zj_width, self.scrollLine.zj_height);
                }
            }
            //添加箭头
...
-(void)drawLineIndicator{
    ...
    if (self.segmentStyle.showLineArrow) {
        //画折线
        CGFloat arrow = 10;
        NSArray *xArr = @[@0, @((self.scrollLine.width-arrow)/2), @(self.scrollLine.width/2), @((self.scrollLine.width+arrow)/2),  @(self.scrollLine.width),@(self.scrollLine.width), @0];
        NSArray *yArr = @[@(self.scrollLine.height/3), @(self.scrollLine.height/3), @(self.scrollLine.height), @(self.scrollLine.height/3), @(self.scrollLine.height/3), @0, @0];
        [self drawLineWithXArr:xArr yArr:yArr atView:self.scrollLine strokeColor:self.segmentStyle.scrollLineArrowColor fillColor:self.segmentStyle.scrollLineArrowColor];
    }
}
//画折线
-(void)drawLineWithXArr:(NSArray *)xArr yArr:(NSArray *)yArr atView:(UIView *)superView strokeColor:(UIColor *)strokeColor fillColor:(UIColor *)fillColor{
    // 线的路径
    UIBezierPath *linePath = [UIBezierPath bezierPath];
    // 起点
    CGFloat x0 = [xArr[0] floatValue];
    CGFloat y0 = [yArr[0] floatValue];
    ...
    //layer
    if (self.scrollLineArrowLayer) {
        [self.scrollLineArrowLayer removeFromSuperlayer];
    }
    self.scrollLineArrowLayer = [CAShapeLayer layer];
    self.scrollLineArrowLayer.lineWidth = 1;
    self.scrollLineArrowLayer.strokeColor = strokeColor.CGColor;
    self.scrollLineArrowLayer.path = linePath.CGPath;
    self.scrollLineArrowLayer.fillColor = fillColor.CGColor; // 默认为blackColor
    
    //    //添加阴影
    //    //lineLayer.masksToBounds=YES;这行去掉
    //    lineLayer.shadowColor=[UIColor blackColor].CGColor;
    //    lineLayer.shadowOffset=CGSizeMake(0, 5);
    //    lineLayer.shadowOpacity=0.5;
    //    lineLayer.shadowRadius=5;
    
    [superView.layer addSublayer:self.scrollLineArrowLayer];
}
